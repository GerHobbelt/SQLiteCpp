...
// -------------------------------------------------------------------------------
int Statement::init_params(Database* paDatabase, const std::string& aQuery)
{
	int ret_val = 0;
	reset_params();

	mQuery = aQuery;
	mStmtPtr.init(paDatabase->getHandle(), mQuery);
...
void Statement::reset_params()
{
	mQuery = std::string("");
	//mStmtPtr; // prepare the SQL query, and ref count (needs Database friendship)
	mColumnCount = 0;
	mbHasRow = false;
	mbDone = false;
}
...
int Statement::Ptr::init(sqlite3* apSQLite, std::string& aQuery)
{
	mpSQLite = apSQLite;
