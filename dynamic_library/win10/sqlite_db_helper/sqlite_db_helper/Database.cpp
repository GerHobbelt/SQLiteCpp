...
int Database::init_v2(const std::string& db_file, std::string& str_err, const int flag /*= SQLite::OPEN_READONLY*/, const int busy_time_out /*= 0*/, const std::string& vfs /*= ""*/)
{
	int ret_val = 0;
	...
	const int ret = sqlite3_open_v2(db_file.c_str(), &handle, flag, vfs.empty() ? nullptr : vfs.c_str());
	// const int ret = sqlite3_open(db_file.c_str(), &handle);// , flag, vfs.empty() ? nullptr : vfs.c_str());
	mSQLitePtr.reset(handle);
...
*	@brief:
*/
int Database::init(const std::string& db_file, std::string& str_err, const int flag /*= SQLite::OPEN_READONLY*/)
{
	int ret_val = 0;
	...
	sqlite3* handle = nullptr;
	const int ret = sqlite3_open(db_file.c_str(), &handle);
	mSQLitePtr.reset(handle);
